...
class GraspDetector(object):
    ...
    def test(self, pcd_path):
        cam_pos = [0, 0, 1]
        input_points = None

        pcl_cloud = pcl.load(pcd_path)
        ...
        for i in ind_good_grasp:
            print(i)
            pose = PoseStamped()
            print(real_grasp[i], score_value[i])
            if (score_value[i] < 0):
                continue
            center = real_grasp[i][0]
...
if __name__ == '__main__':
    ...
    server = GraspDetector()
    server.test(
        "/home/waha/catkin_research_ws/src/grasp_detector/scripts/PybulletGraspSim/output/grasp_detect/pipeline.pcd"
    )
